...
> - 经历从"grep AES S-box"到"差分边界建模"的认知转变过程——以及中间踩过的每一个坑
...
笔者在社区里频繁看到几个概念被混用——有人说”Chrome 自带 VMP”指的是 V8 字节码，有人说”破了 VMP”实际上只是绕过了商业壳的反调试，还有人把 Chrome 沙箱和代码保护混为一谈。这种术语混乱直接导致了大量无效讨论：讨论双方其实在谈完全不同的东西。所以在展开技术分析之前，笔者认为有必要先把边界划清楚 🧑‍🔬 **Human — 概念消歧基于笔者对行业术语混用现状的判断**：
...
笔者在实际分析 CDM 模块的过程中逐渐意识到，很多直觉上"应该能用"的方法之所以失效，根源不在工具不行，而在于分析者没有真正想清楚攻击者和防御者各自拥有什么能力。以下威胁模型就是在多次碰壁之后，笔者回过头来做的一次系统梳理——先把棋盘画出来，再决定落子位置。
...
笔者在分析 CDM 类目标时，和很多分析者一样，最初的直觉是沿着标准密码学结构做搜索。以下是笔者实际踩过的坑——每个”错误假设”都不是凭空列出来的，而是在真实分析中消耗了时间后才认清的 🧑‍🔬 **Human — 失败模式归纳基于笔者实战经验和社区常见误区观察**。

**假设一：”用了 AES 就能搜到 S-box。”** 笔者最初在分析 CDM 时，第一个想法就是扫描标准 AES S-box 的 256 字节序列 `63 7c 77 7b f2 6b 6f c5...`。用 `binwalk` 和自定义脚本在整个 `.data` 和 `.rodata` 段扫了一遍——零命中。扩大到 T-table 的 1024 项特征序列，同样没有。后来才理解：在现代白盒实现中，表可能被编码、拆分成多个小表、甚至在运行时动态生成，标准 S-box 的字节模式根本不会出现在二进制里。

**假设二：”硬件 AES 指令一定会执行。”** 接下来笔者在 Frida 里对 `aesenc` 和 `aesdec` 这两条 AES-NI 指令下了硬件断点。结果：license 处理和解密全程零命中。这些指令确实存在于二进制中，但极可能只是链接时带进来的 BoringSSL 残留代码，真实密钥路径走的是软件白盒变换，完全绕过了硬件 AES。

**假设三：”content key 一定在堆上。”** 笔者在 `UpdateSession` 前后做了两次完整的 heap snapshot diff，试图找到标准 key schedule 的 176 或 240 字节结构。找到了大量新增数据，但没有任何区域匹配标准 key schedule 的熵分布。堆中存在的是 blinded/encoded 状态，裸 key 要么只在栈帧或寄存器中短暂存在，要么已经被 xor mask 保护。

**假设四：”hook 加密库就能拿 key。”** 笔者 hook 了 BoringSSL 的 `AES_encrypt`、`AES_set_encrypt_key` 和 `EVP_DecryptUpdate`，跑了完整的 license + 播放流程——回调函数一次都没有触发。CDM 的白盒路径完全不经过通用密码库，这条线索是条死路。

**假设五：”patch 指令不会影响语义。”** 笔者在 dispatcher 的一个关键分支上打了 `int3` 断点，想观察寄存器状态。结果 license 处理看起来正常通过了，但后续解密完全没有输出——不是 crash，不是报错，只是静默地产出零字节。这正是 `.text` 完整性校验的效果：修改被检测到后，系统走了降级路径，而这个降级路径设计得让你很难判断失败点到底在哪里。

**假设六：”看到热点函数就看到算法。”** 用 `perf record` 采样后发现，超过 60% 的 CPU 时间落在一个地址范围极窄的区域。反汇编后看到的不是 AES 轮结构，而是一个状态机 dispatcher——间接跳转、虚拟寄存器更新、handler 索引。热点不等于算法核心，它只是 VM 调度器的执行入口。
...
这些失败并不意味着目标”无法分析”，而是说明分析层级错了——笔者在六个假设上依次碰壁后才真正接受这一点。面对 VMP，应该少问”哪个函数是 AES”，多问下面几个问题：
...
上一节的六次失败教会笔者一件事：对 VMP 目标，"拿着锤子找钉子"式的单点突破几乎注定无效。标准 S-box 扫描、crypto hook、指令断点——这些都是假设目标保留了标准密码学形状才有效的方法。当这些形状被系统性抹除之后，分析路线本身需要重建。以下方法论正是笔者从那些失败中逐步提炼出来的。
...
例如，笔者在对比两个不同 license 的 heap snapshot 时，发现 `UpdateSession` 后有一块约 16KB 的高熵区域被整体替换——地址相同、大小相同，但内容完全不同。这块区域不像 key schedule（太大），也不像随机 buffer（两次 diff 中偏移结构一致）。这个观察直接指向了 session-derived table 的存在假设，比硬搜 S-box 有效得多。
...
对 VMP 目标，单次观察价值有限。更可靠的是差分。笔者开始认真使用差分方法的契机，是发现用两个不同 key ID 请求 license 时，dispatcher 的 handler 调用序列在前 200 次调用完全一致，但在第 201 次出现分叉——这说明 key selection 逻辑被埋在 VM 调度的某个特定深度，而不是在外层 ABI 可见的分支上。这类发现只有在差分中才浮现：
...
“反编译结果是否难看”不能直接回答系统是否安全。笔者设计下面这个矩阵的起因很具体：在阅读社区里的分析帖时，发现大多数人评价 VMP 强度只看两个指标——handler 数量和 Ghidra 反编译输出的可读性。但笔者在实际分析中发现，反编译输出极度混乱的区域未必是安全关键路径，而某些 Ghidra 能勉强反编译的边界函数反而是真正的薄弱点。所以笔者尝试换一个维度：不评价”代码有多乱”，而是逐项拆解 VMP 对每个安全属性的实际贡献 🧑‍🔬 **Human — 安全属性矩阵由笔者设计，将 VMP 贡献拆解到七项可验证属性**：
...
笔者在阅读社区讨论时发现，大家对”VMP 强度”的评价标准差异极大：有人认为”Ghidra 看不懂就是强保护”，有人认为”只要明文能抓到就是弱保护”，还有人把”需要 Frida”等价于”有 VMP”。这些评价互相矛盾，却都能找到支持案例。根本原因在于缺乏一个基于可观测结果而非主观印象的分级标准。以下四级框架是笔者尝试给出的一种回答 🧑‍🔬 **Human — S0-S3 四级判定框架为笔者提出，用于替代主观”强度”描述**：
...
笔者自己就两种陷阱都踩过。过度乐观那次，是在用 AI 辅助分析 handler 语义时——让 LLM 对 Ghidra 反编译出的 300 多个 handler 做批量命名，输出看起来非常整齐，每个 handler 都有了”AES_round_1””key_expansion_step”之类的标签。笔者差点就信了，直到用差分实验验证时发现，其中近半数标签和实际数据流完全无关——AI 是根据代码结构的表面相似性做的推断，不是根据语义证据。过度悲观那次更早，是在第一次打开 Ghidra 看到 dispatcher 的控制流图时——几千个 basic block 组成的蜘蛛网，笔者的第一反应是”这东西不可分析”。但后来通过 perf 采样和 heap diff 从边界入手，发现其实不需要理解每个 handler 就能定位关键状态变化点。
