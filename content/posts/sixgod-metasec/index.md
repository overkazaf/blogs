...
- **纯算法还原**：有了 unidbg 作为 ground truth，可以逐步将 SM3/Simon/AES 管线替换为纯 Python 实现，最终脱离 native 依赖
- **X-Helios / X-Medusa 深入分析**：这两个参数的加密算法仍未完全理解
- **跨版本分析**：不同抖音版本的 MetaSec SDK 版本号、配置格式和密钥是否变化
...
> 本文所有研究仅用于安全研究和方法论探讨。不公布可用的代码和密钥，如有侵权请联系笔者修改或下架。
